...
use rmp_serde::{decode::from_read as msgpack_read, encode::write as msgpack_write};
use splice::frontends::serde::{SerdeCircuit, SerdeElement, SerdeOption, SerdeSimulation};
...
pub fn bench_msgpack_serialization_long(c: &mut Criterion) {
    ...
    for (rows, cols) in [(5, 5), (10, 10)] {
        ...
        group.bench_with_input(
            BenchmarkId::new("Large circuit", &size_desc),
            &size_desc,
            |b, _| {
                b.iter(|| {
                    let mut buf = Vec::new();
