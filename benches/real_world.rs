...
fn generate_resistor_network(rows: usize, cols: usize) -> String {
    ...
    for row in 0..rows {
        ...
        for col in 0..cols {
            let current_node = if row == 0 && col == 0 {
                1
            } else {
                node_counter - 1
            };
            ...
            if next_node != 0 {
                // Don't connect to ground with resistor
                spice_code.push_str(&format!(
                    "R{}x{} {} {} 1000\n",
...
pub fn bench_resistor_ladder(c: &mut Criterion) {
    ...
    for &size in &sizes {
        ...
        let network =
            create_simulation_from_spice(&spice_code).expect("Failed to parse resistor ladder");

        group.bench_with_input(BenchmarkId::new("Faer Sparse", size), &size, |b, _| {
            b.iter(|| {
                run_sim_for_benchmark::<FaerSparseSolver>(network.clone())
        ...
        group.bench_with_input(BenchmarkId::new("Nalgebra", size), &size, |b, _| {
            b.iter(|| {
                run_sim_for_benchmark::<NalgebraSolver>(network.clone())
                    .expect("Simulation failed");
            });
        });
    }
...
pub fn bench_resistor_network(c: &mut Criterion) {
    ...
    for &(rows, cols) in &configurations {
        ...
        let network =
            create_simulation_from_spice(&spice_code).expect("Failed to parse resistor network");

        group.bench_with_input(
            BenchmarkId::new("Faer Sparse", &size_desc),
            &size_desc,
            |b, _| {
                b.iter(|| {
                    run_sim_for_benchmark::<FaerSparseSolver>(network.clone())
        ...
        group.bench_with_input(
            BenchmarkId::new("Nalgebra", &size_desc),
            &size_desc,
            |b, _| {
                b.iter(|| {
                    run_sim_for_benchmark::<NalgebraSolver>(network.clone())
