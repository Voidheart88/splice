...
use crate::frontends::serde::{SerdeCircuit, SerdeElement, SerdeOption, SerdeSimulation};
...
fn test_network_backend_basic() {
    ...
    let results = SimulationResults {
        options: vec![],
        results: vec![Sim::Op(vec![(
            Variable::new(Arc::from("n1"), crate::models::Unit::Volt, 0),
            5.0,
        )])],
    };
...
fn test_network_dc_analysis() {
    ...
    let circuit = SerdeCircuit {
        ...
        simulations: vec![crate::frontends::serde::SerdeSimulation::DC(
            crate::frontends::serde::SerdeDC::new("V1".to_string(), 0.0, 10.0, 2.5),
        )],
        options: vec![SerdeOption {
            out: "n1".to_string(),
        }],
    ...
    match frontend_result {
        Ok(frontend) => {
            ...
            if let crate::sim::commands::SimulationCommand::Dc(source, start, stop, step, _) =
                &sim.commands[0]
            {
