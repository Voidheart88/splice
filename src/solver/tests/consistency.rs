...
fn test_simple_2x2_system() -> Result<(), String> {
    ...
    for (i, solver) in solvers.iter_mut().enumerate() {
        ...
        let result = solver
            .solve()
            .map_err(|e| format!("Solver {} failed: {:?}", i, e))?;
    ...
    for (i, result) in results.iter().enumerate().skip(1) {
        ...
        assert!(
            x_diff < tolerance,
            "Solver {} x-value differs by {} from solver 0",
            i,
            x_diff
        );
        assert!(
            y_diff < tolerance,
            "Solver {} y-value differs by {} from solver 0",
            i,
            y_diff
        );
    }
...
fn test_simple_3x3_system() -> Result<(), String> {
    ...
    for (i, result) in results.iter().enumerate().skip(1) {
        for j in 0..3 {
            ...
            assert!(
                diff < tolerance,
                "Solver {} value {} differs by {} from solver 0",
                i,
                j,
                diff
            );
    ...
    for (i, solver) in solvers.iter_mut().enumerate() {
        ...
        match result {
            Ok(solution) => {
                ...
                println!(
                    "Solver {} succeeded: x = {}, y = {}",
                    i, solution[0], solution[1]
                );
            }
...
fn test_circuit_like_system() -> Result<(), String> {
    ...
    for (i, result) in results.iter().enumerate().skip(1) {
        for j in 0..3 {
            ...
            assert!(
                diff < tolerance,
                "Solver {} value {} differs from solver 0 by {}",
                i,
                j,
                diff
            );
...
fn test_performance_consistency() {
    ...
    for &size in &sizes {
        ...
        for (i, solver) in solvers.iter_mut().enumerate() {
            ...
            println!(
                "Solver {}: {:.6}s average for {}x{} system",
                i, avg_time, size, size
            );
        }
    }
}
