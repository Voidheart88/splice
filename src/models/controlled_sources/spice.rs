...
use crate::{Element, FrontendError, Variable};
...
pub fn process_vcvs(
    ...
    let name = inner
        .next()
        .ok_or_else(|| FrontendError::ParseError("Missing name for VCVS source".into()))?;
    let name_str = name.as_str();

    // Parse nodes
    let pos_node = inner
        .next()
        .ok_or_else(|| FrontendError::ParseError("Missing positive node for VCVS source".into()))?;
    let neg_node = inner
        .next()
        .ok_or_else(|| FrontendError::ParseError("Missing negative node for VCVS source".into()))?;
    ...
    // Parse gain
    let gain_value = inner
        .next()
        .ok_or_else(|| FrontendError::ParseError("Missing gain value for VCVS source".into()))?;
    let gain: Numeric = gain_value
        .as_str()
        .parse()
        .map_err(|_| FrontendError::ParseError("Invalid gain value for VCVS source".into()))?;
    ...
    let neg_var = Variable::new(
        Arc::from(neg_node.as_str()),
        Unit::Volt,
        variables.len() + 1,
    );
    let ctrl_pos_var = Variable::new(
        Arc::from(ctrl_pos_node.as_str()),
        Unit::Volt,
        variables.len() + 2,
    );
    let ctrl_neg_var = Variable::new(
        Arc::from(ctrl_neg_node.as_str()),
        Unit::Volt,
        variables.len() + 3,
    );
...
pub fn process_vccs(
    ...
    let name = inner
        .next()
        .ok_or_else(|| FrontendError::ParseError("Missing name for VCCS source".into()))?;
    let name_str = name.as_str();

    // Parse nodes
    let pos_node = inner
        .next()
        .ok_or_else(|| FrontendError::ParseError("Missing positive node for VCCS source".into()))?;
    let neg_node = inner
        .next()
        .ok_or_else(|| FrontendError::ParseError("Missing negative node for VCCS source".into()))?;
    let ctrl_pos_node = inner.next().ok_or_else(|| {
...
pub fn process_cccs(
    ...
    let name = inner
        .next()
        .ok_or_else(|| FrontendError::ParseError("Missing name for CCCS source".into()))?;
    let name_str = name.as_str();

    // Parse nodes
    let pos_node = inner
        .next()
        .ok_or_else(|| FrontendError::ParseError("Missing positive node for CCCS source".into()))?;
    let neg_node = inner
        .next()
        .ok_or_else(|| FrontendError::ParseError("Missing negative node for CCCS source".into()))?;
    ...
    let gain_value = inner
        .next()
        .ok_or_else(|| FrontendError::ParseError("Missing gain value for CCCS source".into()))?;
    let gain: Numeric = gain_value
        .as_str()
        .parse()
        .map_err(|_| FrontendError::ParseError("Invalid gain value for CCCS source".into()))?;
    ...
    let ctrl_branch_var = Variable::new(
        Arc::from(ctrl_branch_node.as_str()),
        Unit::Volt,
        variables.len() + 2,
    );
...
pub fn process_ccvs(
    ...
    let name = inner
        .next()
        .ok_or_else(|| FrontendError::ParseError("Missing name for CCVS source".into()))?;
    let name_str = name.as_str();

    // Parse nodes
    let pos_node = inner
        .next()
        .ok_or_else(|| FrontendError::ParseError("Missing positive node for CCVS source".into()))?;
    let neg_node = inner
        .next()
        .ok_or_else(|| FrontendError::ParseError("Missing negative node for CCVS source".into()))?;
    let ctrl_branch_node = inner.next().ok_or_else(|| {
    ...
    let gain_value = inner
        .next()
        .ok_or_else(|| FrontendError::ParseError("Missing gain value for CCVS source".into()))?;
    let gain: Numeric = gain_value
        .as_str()
        .parse()
        .map_err(|_| FrontendError::ParseError("Invalid gain value for CCVS source".into()))?;
