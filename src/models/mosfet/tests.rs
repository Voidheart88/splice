...
fn test_mosfet_triples_basic() {
    ...
    let mosfet = Mos0Bundle::new(Arc::from("M1"), Some(gate), Some(drain), Some(source), None);
    ...
    for (_, _, value) in triples_data.iter() {
        if *value != 0.0 {
            ...
            assert!(
                value.abs() < 1e-4,
                "Conductance should be very small at zero voltage, got {}",
                value
            );
        }
    }
...
fn test_mosfet_pairs_basic() {
    ...
    for (_, value) in pairs_data.iter() {
        if *value != 0.0 {
            ...
            assert!(
                value.abs() < 1e-4,
                "Current should be very small at zero voltage, got {}",
                value
            );
        }
    }
...
fn test_mosfet_above_threshold() {
    ...
    let drain_current = pairs_data
        .iter()
        .find(|(idx, _)| *idx == 1)
        .map(|(_, val)| *val)
        .unwrap_or(0.0);

    let source_current = pairs_data
        .iter()
        .find(|(idx, _)| *idx == 2)
        .map(|(_, val)| *val)
        .unwrap_or(0.0);
    ...
    assert!(
        drain_current > 0.0,
        "Drain current should be positive when conducting, got {}",
        drain_current
    );
    assert!(
        source_current < 0.0,
        "Source current should be negative when conducting, got {}",
        source_current
    );
    ...
    assert!(
        current_diff < 1e-6,
        "Drain and source currents should be equal, difference: {}",
        current_diff
    );
}
...
fn test_mosfet_triple_idx() {
    ...
    assert!(
        triple_idx.is_some(),
        "Should have triple indices for full MOSFET"
    );
...
fn test_mosfet_partial_connections() {
    ...
    assert!(
        non_zero_count >= 1,
        "Should have at least 1 current for partial connections, got {}",
        non_zero_count
    );
}
...
fn test_mosfet_numerical_stability() {
    ...
    for (_, value) in pairs_data.iter() {
        assert!(
            value.is_finite(),
            "Should not produce NaN or infinite values, got {}",
            value
        );
    }
}
...
fn test_mosfet_saturation_region() {
    ...
    assert!(
        drain_current > 1e-5,
        "Should have current in saturation, got {}",
        drain_current
    );
}
...
fn test_mosfet_linear_region() {
    ...
    assert!(
        drain_current > 0.0,
        "Should have current in linear region, got {}",
        drain_current
    );
}
...
fn test_mosfet_subthreshold() {
    ...
    // The MOSFET model produces small currents even below threshold
    assert!(
        drain_current.abs() < 1e-4,
        "Should have small current in subthreshold, got {}",
        drain_current
    );
}
...
fn test_mosfet_symmetry() {
    ...
    let drain_current1 = pairs1
        .data()
        .iter()
        .find(|(idx, _)| *idx == 1)
        .map(|(_, val)| *val)
        .unwrap_or(0.0);
    let source_current1 = pairs1
        .data()
        .iter()
        .find(|(idx, _)| *idx == 2)
        .map(|(_, val)| *val)
    ...
    let drain_current2 = pairs2
        .data()
        .iter()
        .find(|(idx, _)| *idx == 1)
        .map(|(_, val)| *val)
        .unwrap_or(0.0);
    let source_current2 = pairs2
        .data()
        .iter()
        .find(|(idx, _)| *idx == 2)
        .map(|(_, val)| *val)
    ...
    eprintln!(
        "Original: Id = {}, Is = {}",
        drain_current1, source_current1
    );
    ...
    assert!(
        current_diff1 < 1e-6,
        "KCL should hold for original configuration, difference: {}",
        current_diff1
    );
    assert!(
        current_diff2 < 1e-6,
        "KCL should hold for swapped configuration, difference: {}",
        current_diff2
    );
}
