...
impl Bjt0Bundle {
    ...
    pub fn triples(&self, x_vec: &[Numeric]) -> Triples<Numeric, 16> {
        ...
        let g_be = if v_be > -100.0 {  // Avoid numerical overflow
            (is * (Numeric::exp(v_be / (n * UT)) - Numeric::one())) / (n * UT)
        } else {
            Numeric::zero()
        };
        
        let g_bc = if v_bc > -100.0 {  // Avoid numerical overflow
    ...
    pub fn pairs(&self, x_vec: &[Numeric]) -> Pairs<Numeric, 3> {
        ...
        let i_be = if v_be > -100.0 {  // Avoid numerical overflow
            is * (Numeric::exp(v_be / (n * UT)) - Numeric::one())
        } else {
            Numeric::zero()
        };
        
        let i_bc = if v_bc > -100.0 {  // Avoid numerical overflow
        ...
        match (b_idx, c_idx, e_idx) {
            // All three terminals connected
            (Some(b), Some(c), Some(e)) => {
                // Ebers-Moll model: Correct implementation
                // I_BE = I_S * (exp(V_BE/(n*UT)) - 1)  - Base-Emitter diode current
                // I_BC = I_S * (exp(V_BC/(n*UT)) - 1)  - Base-Collector diode current  
                // I_C = beta * I_BE - I_BC            - Collector current (flows OUT of collector)
                // I_B = I_BE + I_BC - I_C/beta        - Base current (flows INTO base)
                // I_E = I_BE + I_C/beta               - Emitter current (flows INTO emitter)
    ...
    pub fn triple_idx(&self) -> Option<TripleIdx<12>> {
        ...
        match (b_idx, c_idx, e_idx) {
            ...
            (Some(b), Some(c), Some(e)) => Some(TripleIdx::new(&[
                (b, b), (e, e), (b, e), (e, b),  // BE diode
                (b, b), (c, c), (b, c), (c, b),  // BC diode
                (c, b), (b, c),                  // Transistor action
                (c, e), (e, c),                  // CE path
            ])),
            
            // Base and Collector connected, Emitter grounded
            (Some(b), Some(c), None) => Some(TripleIdx::new(&[
                (b, b), (c, c), (b, c), (c, b),
            ])),
            
            // Base and Emitter connected, Collector grounded
            (Some(b), None, Some(e)) => Some(TripleIdx::new(&[
                (b, b), (e, e), (b, e), (e, b),
            ])),
            
            // Collector and Emitter connected, Base grounded
            (None, Some(c), Some(e)) => Some(TripleIdx::new(&[
                (c, c), (e, e),
            ])),
